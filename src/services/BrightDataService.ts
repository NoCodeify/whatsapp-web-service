...
export class BrightDataService {
  private logger = pino({ name: "BrightDataService" });
  private apiClient: AxiosInstance;
  private firestore: Firestore;
  private staticIPs: Map<string, StaticIP> = new Map();
   ...
   */
  private async syncStaticIPs(): Promise<void> {
    try {
   ...
   */
  private generateSessionBasedIPs(): void {
   ...
   */
  private generateSessionBasedIP(): string {
  ...
  async assignStaticIP(
    ...
    let availableIP: StaticIP | null = null;
   ...
   */
  async releaseIP(phoneNumber: string): Promise<void> {
   ...
   */
  async getProxyConfig(userId: string, phoneNumber: string): Promise<any> {
  ...
  async testConnection(phoneNumber?: string): Promise<boolean> {
    try {
      ...
      const response = await axios.get("https://geo.brdtest.com/welcome.txt", {
        proxy: {
