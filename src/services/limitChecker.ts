...
export class LimitChecker {
   ...
   */
  private async checkIfNewContact(userId: string, recipientNumber: string): Promise<boolean> {
    try {
      // Get user reference for querying
      const userRef = this.db.collection("users").doc(userId);

      // Check if contact exists in GLOBAL contacts collection (not subcollection!)
      const contactsQuery = await this.db.collection("contacts").where("user", "==", userRef).where("phone_number", "==", recipientNumber).limit(1).get();
      ...
      logger.error({ error, userId, recipientNumber }, "Error checking if new contact");
   ...
   */
  private resetDailyCountersIfNeeded(usage: WhatsAppWebUsage): WhatsAppWebUsage {
    const today = new Date().toISOString().split("T")[0];

    if (usage.today_date !== today) {
      logger.info({ oldDate: usage.today_date, newDate: today }, "Resetting daily counters");
      return {
        ...usage,
        today_date: today,
   ...
   */
  async checkLimits(userId: string, phoneNumber: string, recipientNumber: string): Promise<LimitCheckResult> {
    ...
    const phoneDocRef = this.db.collection("users").doc(userId).collection("phone_numbers").doc(phoneNumber);
    ...
    try {
      const result = await this.db.runTransaction(async (transaction) => {
        ...
        usage.total_messages_today = this.sanitizeCounter(usage.total_messages_today);
        ...
        const dailyMessageLimit = Math.max(1, this.sanitizeCounter(phoneData?.daily_message_limit) || 250);
        ...
        if (isNewContact) {
          if (usage.new_contacts_today >= dailyLimit) {
            ...
            const totalMessageLimitReached = totalMessagesUsed >= dailyMessageLimit;
            ...
            return {
              ...
              totalMessagesUsage: {
                ...
                percentage: Math.min(100, (totalMessagesUsed / dailyMessageLimit) * 100),
              },
        ...
        // Safe increment: sanitize before adding (prevents string concatenation)
        usage.total_messages_today = this.sanitizeCounter(usage.total_messages_today) + 1;
        ...
        const totalMessageLimitReached = totalMessagesUsed >= dailyMessageLimit;
        const shouldSendEmail = totalMessageLimitReached && !usage.limit_email_sent_today;
        const totalMessagesRemaining = Math.max(0, dailyMessageLimit - totalMessagesUsed);
        const totalMessagesPercentage = Math.min(100, (totalMessagesUsed / dailyMessageLimit) * 100);
