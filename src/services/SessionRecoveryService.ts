...
export class SessionRecoveryService {
  ...
  private readonly options: RecoveryOptions = {
    ...
    priorityCountries: process.env.PRIORITY_COUNTRIES?.split(",") || ["us", "gb", "de"],
  };

  constructor(firestore: Firestore, instanceId?: string) {
    this.firestore = firestore;
    this.instanceId = instanceId || `instance_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  async recoverActiveSessions(): Promise<void> {
    ...
    try {
      ...
      await this.firestore.collection("server_instances").doc(this.instanceId).update({
  ...
  private async getActiveSessionsToRecover(): Promise<RecoverySession[]> {
    ...
    try {
      this.logger.info("Searching for WhatsApp Web sessions to recover using collectionGroup query");
      ...
      const phoneNumbersSnapshot = await this.firestore.collectionGroup("phone_numbers").where("type", "==", "whatsapp_web").get();
      ...
      phoneNumbersSnapshot.forEach((phoneDoc) => {
        ...
        if (!userId) {
          this.logger.warn({ phoneNumber }, "Could not extract userId from document path");
          return;
        }
        ...
        if (!hasSessionData) {
          this.logger.debug({ userId, phoneNumber }, "Skipping session recovery - no valid session data or QR scan");
          return;
        }
        ...
        if (recoveryStatuses.includes(sessionStatus)) {
          sessions.push({
            ...
            lastConnected: data.whatsapp_web?.last_updated?.toDate() || data.updated_at?.toDate() || new Date(),
            status: "disconnected",
          });

          this.logger.debug({ userId, phoneNumber, status: sessionStatus }, "Added session for recovery");
        } else {
          this.logger.debug({ userId, phoneNumber, status: sessionStatus }, "Skipping session - status not eligible for recovery");
        }
      });

      if (sessions.length > 0) {
        this.logger.info({ found: sessions.length }, "Found sessions in unified phone_numbers collection for recovery");
      } else {
  ...
  private prioritizeSessions(sessions: RecoverySession[]): RecoverySession[] {
    return sessions.sort((a, b) => {
      ...
      const aPriority = this.options.priorityCountries?.indexOf(a.phoneCountry || "") ?? -1;
      const bPriority = this.options.priorityCountries?.indexOf(b.phoneCountry || "") ?? -1;
  ...
  private async recoverSession(session: RecoverySession): Promise<void> {
    const { userId, phoneNumber, proxyIp, phoneCountry, proxyCountry } = session;
    ...
    if (this.instanceCoordinator) {
      ...
      if (!shouldHandle) {
        this.logger.info({ userId, phoneNumber, instanceId: this.instanceId }, "Session is being handled by another instance, skipping recovery");
        return;
      }
    }

    this.logger.info({ userId, phoneNumber, proxyIp, phoneCountry, proxyCountry }, "Recovering session");
    ...
    while (attempts < this.options.maxReconnectAttempts && !recovered) {
      ...
      try {
        ...
        if (connected) {
          ...
          this.logger.info({ userId, phoneNumber, attempts }, "Session recovered successfully");
        } else {
   ...
   */
  private async updateAllSessionStatuses(userId: string, phoneNumber: string, status: string): Promise<void> {
    try {
      ...
      if (!phoneDoc.exists) {
        this.logger.info({ userId, phoneNumber }, "Phone number document doesn't exist (was deleted), skipping status update to respect deletion");
        return;
      }
      ...
      this.logger.info({ userId, phoneNumber, status: firestoreStatus }, "Updated session status in unified phone_numbers collection");
    } catch (error: any) {
      this.logger.error({ error: error.message, userId, phoneNumber, status }, "Failed to update session status in unified collection");
    }
  }
  ...
  async cleanupOldInstances(): Promise<void> {
    try {
      ...
      const oldInstances = await this.firestore.collection("server_instances").where("startedAt", "<", Timestamp.fromDate(cutoff)).get();
  ...
  async cleanupStaleSessions(): Promise<void> {
    try {
      ...
      const staleSessions = await this.firestore
        ...
        .where("whatsapp_web.last_activity", "<", Timestamp.fromDate(staleThreshold))
        .get();
      ...
      if (staleSessions.size > 0) {
        await batch1.commit();
        this.logger.info({ count: staleSessions.size }, "Cleaned up stale sessions older than 72 hours");
      }
      ...
      if (failedSessions.size > 0) {
        await batch2.commit();
        this.logger.info({ count: failedSessions.size }, "Cleaned up failed sessions older than 24 hours");
      }
      ...
      const oldPendingSessions = await this.firestore
        ...
        .where("whatsapp_web.last_activity", "<", Timestamp.fromDate(new Date(now.getTime() - 6 * 60 * 60 * 1000)))
        .get();
      ...
      oldPendingSessions.forEach((doc) => {
        ...
        if (!data.session_exists && !data.qr_scanned) {
          ...
          this.logger.info({ userId, phoneNumber: data.phone_number }, "Preserved pending_recovery session with valid data for recovery");
        }
        cleanupCount++;
      });
      ...
      if (cleanupCount > 0) {
        this.logger.info({ totalCleaned: cleanupCount }, "Session cleanup completed");
      }
    } catch (error: any) {
      this.logger.error({ error: error.message }, "Failed to cleanup stale sessions");
    }
  }
