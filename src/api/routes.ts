...
const authenticate = (req: AuthenticatedRequest, res: Response, next: Function) => {
...
export function createApiRoutes(
  ...
  router.post("/sessions/initialize", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const { phoneNumber, proxyCountry, countryCode, browserName } = req.body;
      ...
      if (!formattedPhone) {
        logger.warn({ userId, phoneNumber, countryCode }, "Invalid phone number format");
        return res.status(400).json({
          error: "Invalid phone number format",
          message: "Please provide a valid phone number with country code (e.g., +31612345678)",
        });
      }

      logger.info({ userId, original: phoneNumber, formatted: formattedPhone }, "Formatted phone number");
      ...
      const added = await connectionPool.addConnection(userId, formattedPhone, proxyCountry, countryCode, false, browserName);
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to initialize session");
   ...
   */
  router.get("/sessions/:userId/qr", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      ...
      logger.error({ error, userId: req.params.userId }, "Failed to get QR code");
   ...
   */
  router.get("/sessions/:userId/status", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const { userId } = req.params;
      const phoneNumber = req.query.phoneNumber as string;
   ...
   */
  router.delete("/sessions/:userId", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const { userId } = req.params;
      const phoneNumber = req.query.phoneNumber as string;
      ...
      const formattedPhone = formatPhoneNumberSafe(phoneNumber);
      if (!formattedPhone) {
        logger.warn({ userId, phoneNumber }, "Invalid phone number format in DELETE request");
        return res.status(400).json({
          error: "Invalid phone number format",
      ...
      logger.info({ userId, original: phoneNumber, formatted: formattedPhone }, "Processing session deletion request");
      ...
      logger.error({ error, userId: req.params.userId }, "Failed to disconnect session");
  ...
  if (reconnectionService) {
     ...
     */
    router.get("/sessions/can-reconnect", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
      try {
        const phoneNumber = req.query.phoneNumber as string;
        const userId = req.user!.userId;
        ...
        const result = await reconnectionService.canReconnect(userId, phoneNumber);

        res.json(result);
      } catch (error) {
        logger.error({ error, userId: req.user?.userId }, "Failed to check reconnection status");
        return res.status(500).json({ error: "Failed to check reconnection status" });
      }
    });
     ...
     */
    router.post("/sessions/reconnect", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
      try {
        ...
        logger.info({ userId, phoneNumber, forceNew }, "Reconnection attempt requested");

        const result = await reconnectionService.reconnect(userId, phoneNumber, forceNew);
        ...
        logger.error({ error, userId: req.user?.userId }, "Failed to reconnect session");
     ...
     */
    router.get("/sessions/reconnect-status", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
      try {
        const phoneNumber = req.query.phoneNumber as string;
        const userId = req.user!.userId;
        ...
        const existingConnection = connectionPool.getConnection(userId, phoneNumber);
        ...
        const sessionExists = await sessionManager.sessionExists(userId, phoneNumber);
        ...
        logger.error({ error, userId: req.user?.userId }, "Failed to get reconnection status");
        return res.status(500).json({ error: "Failed to get reconnection status" });
      }
    });
     ...
     */
    router.get("/sessions/reconnection-stats", async (_req: AuthenticatedRequest, res: Response): Promise<any> => {
      try {
        ...
        return res.status(500).json({ error: "Failed to get reconnection stats" });
      }
    });
  }
   ...
   */
  router.post("/messages/check-limits", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      ...
      // Check limits
      const limitCheck = await limitChecker.checkLimits(userId, phoneNumber, toNumber);
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to check limits");
   ...
   */
  router.post("/messages/send", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      const limitCheck = await limitChecker.checkLimits(userId, formattedPhone, formattedToNumber);
      ...
      if (!limitCheck.allowed) {
        logger.warn({ userId, phoneNumber, toNumber, error: limitCheck.error }, "Message blocked by limits");
      ...
      const messageKey = await connectionPool.sendMessage(userId, formattedPhone, formattedToNumber, content);
      ...
      if (!messageKey) {
        ...
        return res.status(500).json({
          ...
          reason: "no_message_key_returned",
          details: "WhatsApp socket did not return a message key. The connection may be disconnected or the recipient number may be invalid.",
          correlationId,
        });
      }
      ...
      logger.info(
        {
          ...
          sendDuration,
          totalDuration,
          creditsUsed: (limitCheck.usage as any)?.messagesUsedToday || limitCheck.usage?.used,
          isNewContact: limitCheck.isNewContact,
          timestamp: new Date().toISOString(),
        },
   ...
   */
  router.post("/messages/typing", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to send typing indicator");
   ...
   */
  router.post("/messages/read", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to mark messages as read");
   ...
   */
  router.get("/proxy/locations", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
   ...
   */
  router.post("/connections/recover", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      logger.info({ userId: req.user?.userId }, "Manual connection recovery requested");
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to initiate recovery");
      return res.status(500).json({
   ...
   */
  router.get("/connections/status", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const poolMetrics = connectionPool.getMetrics();
      ...
      if (connectionStateManager) {
        ...
        activeConnections = await connectionStateManager.getActiveConnections();
      }
      ...
      logger.error({ error, userId: req.user?.userId }, "Failed to get connection status");
      return res.status(500).json({
   ...
   */
  router.get("/proxy/status", async (_req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
   ...
   */
  router.post("/proxy/test", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const { phoneNumber } = req.body;
      const userId = req.user!.userId;

      const testResult = await proxyManager.testProxyConnection(userId, phoneNumber || "test_connection");

      res.json({
        success: testResult,
        message: testResult ? "Proxy connection successful" : "Proxy connection failed - check configuration",
      });
    } catch (error) {
   ...
   */
  router.post("/proxy/rotate", async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
      const { phoneNumber } = req.body;
      const userId = req.user!.userId;
      ...
      const newConfig = await proxyManager.rotateProxy(userId, formattedPhone);
