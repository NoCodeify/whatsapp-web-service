...
async function runAutoReconnectionTests() {
  ...
  try {
    ...
    const activeSessions = await createActiveTestSessions(
      dynamicProxyService,
      firestore,
    );
...
async function verifyProxyReactivation(
  ...
  for (const session of sessions) {
    try {
      ...
      logger.error(
        { error: error.message },
        "Failed to verify proxy reactivation",
      );
    }
  }
  ...
  for (const collectionName of collections) {
    try {
      ...
      if (!snapshot.empty) {
        await batch.commit();
        logger.info(
          `Cleaned ${snapshot.size} documents from ${collectionName}`,
        );
      }
    } catch (error: any) {
      ...
      try {
        ...
        testDocs.forEach((doc) => {
          ...
          if (
            ...
            data.instanceId?.startsWith("test_")
          ) {
            batch.delete(doc.ref);
          }
        });
