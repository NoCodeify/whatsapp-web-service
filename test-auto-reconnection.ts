...
async function runAutoReconnectionTests() {
  ...
  const sessionRecoveryService = new SessionRecoveryService(firestore, dynamicProxyService, "test_instance_main");
  ...
  try {
    ...
    const newRecoveryService = new SessionRecoveryService(firestore, dynamicProxyService, "test_instance_restart");
...
async function createActiveTestSessions(
  ...
  for (const scenario of TEST_SCENARIOS) {
    try {
      // Assign proxy
      const result = await proxyService.assignProxy(scenario.userId, scenario.phoneNumber, scenario.country);

      // Create WhatsApp phone number record
      await firestore
        .collection("whatsapp_phone_numbers")
        .doc(`${scenario.userId}_${scenario.phoneNumber}`)
        .set({
      ...
      logger.error({ error: error.message, scenario: scenario.name }, "Failed to create test session");
    }
  }
 ...
 */
async function verifyProxyReactivation(firestore: Firestore, sessions: Array<{ userId: string; phoneNumber: string; proxyIp: string }>): Promise<void> {
  let reactivated = 0;
  let newProxies = 0;
  ...
  for (const session of sessions) {
    try {
      ...
      const proxyDoc = await firestore.collection("proxy_inventory").doc(session.proxyIp).get();

      if (proxyDoc.exists && proxyDoc.data()?.status === "active") {
        reactivated++;
        logger.info({ ip: session.proxyIp }, "✅ Proxy successfully reactivated");
      } else {
        // Check if new proxy was assigned
        const assignmentDoc = await firestore.collection("proxy_assignments").doc(`${session.userId}_${session.phoneNumber}`).get();

        if (assignmentDoc.exists) {
          const newIp = assignmentDoc.data()?.proxyIp;
          if (newIp !== session.proxyIp) {
            newProxies++;
            logger.info({ oldIp: session.proxyIp, newIp }, "✅ New proxy assigned (original unavailable)");
          }
        }
      }
    } catch (error: any) {
  ...
  logger.info({ reactivated, newProxies, total: sessions.length }, "Proxy reactivation summary");
}

/**
 * Simulate crash scenario
 */
async function simulateCrashScenario(firestore: Firestore, proxyService: DynamicProxyService): Promise<void> {
  ...
  await firestore.collection("whatsapp_phone_numbers").doc(`${crashUserId}_${crashPhone}`).set({
    user_id: crashUserId,
  ...
  const crashRecoveryService = new SessionRecoveryService(firestore, proxyService, "test_crash_recovery_instance");

  // Should recover the session
  await crashRecoveryService.recoverActiveSessions();

  // Verify recovery
  const recoveryDoc = await firestore.collection("session_recovery").doc(`${crashUserId}_${crashPhone}`).get();
 ...
 */
async function testMultiInstanceStartup(firestore: Firestore, proxyService: DynamicProxyService): Promise<void> {
  ...
  const recoveryPromises = instances.map((instance) =>
    instance.recoverActiveSessions().catch((err) => logger.warn({ error: err.message }, "Instance recovery failed"))
  );

  await Promise.all(recoveryPromises);

  // Check server instances
  const instancesSnapshot = await firestore.collection("server_instances").get();

  const runningInstances = instancesSnapshot.docs.filter((doc) => doc.data().status === "running");

  logger.info({ count: runningInstances.length }, "Multi-instance startup completed");
}

/**
 * Test proxy availability changes
 */
async function testProxyAvailabilityChanges(firestore: Firestore, proxyService: DynamicProxyService): Promise<void> {
...
async function cleanupTestData(firestore: Firestore): Promise<void> {
  ...
  const collections = ["proxy_assignments", "proxy_inventory", "whatsapp_phone_numbers", "session_recovery", "server_instances"];

  for (const collectionName of collections) {
    try {
      const snapshot = await firestore.collection(collectionName).where("userId", ">=", "test_").get();
      ...
      try {
        const testDocs = await firestore.collection(collectionName).get();
        ...
        logger.warn({ error: cleanupError, collection: collectionName }, "Failed to cleanup collection");
      }
    }
  }
}
