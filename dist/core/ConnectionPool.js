...
class ConnectionPool extends events_1.EventEmitter {
    ...
    constructor(proxyManager, sessionManager, firestore, pubsub, connectionStateManager) {
        super();
        ...
        this.wsManager = new CloudRunWebSocketManager_1.CloudRunWebSocketManager();
        this.memoryLeakPrevention = new MemoryLeakPrevention_1.MemoryLeakPrevention();
        this.errorHandler = new ErrorHandler_1.ErrorHandler();
        this.instanceCoordinator = new InstanceCoordinator_1.InstanceCoordinator(firestore);
     ...
     */
    setupWebSocketManagerListeners() {
        ...
        this.errorHandler.on("websocket-recovery-needed", async ({ connectionId, userId, phoneNumber }) => {
            ...
            if (userId && phoneNumber) {
                try {
                    ...
                    await this.errorHandler.executeWithRetry(() => this.addConnection(userId, phoneNumber), {
                        userId,
                        phoneNumber,
                        connectionId,
     ...
     */
    setupInstanceCoordinatorListeners() {
        ...
        try {
            ...
            for (const session of filesystemSessions) {
                ...
                try {
                    ...
                    const success = await this.addConnection(userId, phoneNumber, undefined, // No proxy country for recovery
     ...
     */
    async addConnection(userId, phoneNumber, proxyCountry, countryCode, isRecovery = false, browserName) {
        // Format the phone number to ensure consistent E.164 format
        const formattedNumber = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber, countryCode);
        if (!formattedNumber) {
        ...
        if (this.connections.has(connectionKey)) {
            ...
            if (existing.state.connection === "open" ||
        ...
        try {
            ...
            await this.createPhoneNumberRecord(userId, phoneNumber, countryCode || proxyCountry);
            // Create connection with proxy and custom browser name
            const socket = await this.sessionManager.createConnection(userId, phoneNumber, proxyCountry, browserName);
            const connection = {
                userId,
                phoneNumber,
                socket,
            ...
            this.logger.info({ userId, phoneNumber, totalConnections: this.connections.size }, "Added new connection to pool");
            return true;
        }
        catch (error) {
    ...
    setupConnectionHandlers(connection) {
        ...
        this.logger.debug({ userId, phoneNumber }, "Connection registered with WebSocket manager and memory leak prevention");
        ...
        socket.ev.on("connection.update", async (update) => {
            ...
            if (qr) {
                ...
                connection.qrTimeout = setTimeout(async () => {
                    if (connection.state.connection !== "open") {
                        this.logger.warn({ userId, phoneNumber }, "QR code expired without connection - removing connection to prevent proxy leak");
                        await this.removeConnection(userId, phoneNumber);
                    }
            ...
            if (state === "open") {
                ...
                this.logger.info({ userId, phoneNumber }, "WhatsApp connection established");
                // Emit sync started event with small delay to ensure WebSocket clients are ready
                setTimeout(() => {
                    this.logger.info({ userId, phoneNumber }, "Emitting sync:started event");
                ...
                setTimeout(() => {
                    ...
                    if (!syncCompleted) {
                        syncCompleted = true;
                        this.logger.info({ userId, phoneNumber }, "Sync timeout reached, completing sync");
            ...
            if (state === "close") {
                const disconnectReason = lastDisconnect?.error?.output
                    ?.statusCode;
                // Handle expected restart after QR pairing (error code 515)
                if (disconnectReason === baileys_1.DisconnectReason.restartRequired) {
                    this.logger.info({ userId, phoneNumber }, "Connection restart required after pairing - this is expected");
                ...
                if (disconnectReason === baileys_1.DisconnectReason.connectionReplaced) {
                    this.logger.warn({ userId, phoneNumber, disconnectReason }, "Connection replaced - session is active elsewhere, not reconnecting");
                ...
                if (shouldReconnect) {
                    ...
                    try {
                        ...
                        const errorHandled = await this.errorHandler.handleError(lastError ||
            ...
            try {
                ...
                for (const msg of upsert.messages) {
                    ...
                    try {
                        ...
                        await this.errorHandler.handleError(messageError, {
                            userId,
                            phoneNumber,
                            connectionId,
            ...
            try {
                for (const update of updates) {
                    try {
                        ...
                        await this.errorHandler.handleError(updateError, {
                            userId,
                            phoneNumber,
                            connectionId,
        ...
        socket.ev.on("messages.upsert", async (upsert) => {
            ...
            if (upsert.type === "append" || upsert.type === "notify") {
                ...
                if (oldMessages.length > 0) {
                    ...
                    const count = await this.processSyncedMessages(userId, phoneNumber, oldMessages);
                    totalMessagesSynced += count;
                    // Emit progress event
                    this.emit("sync:progress", {
                        userId,
    ...
    async handleIncomingMessage(userId, phoneNumber, message) {
        try {
            ...
            if (existingContacts.empty) {
                ...
                this.logger.info({ userId, phoneNumber, fromNumber: formattedFromPhone }, "Created new contact from incoming message");
            }
            else {
            ...
            this.logger.error({ userId, phoneNumber, error }, "Failed to handle incoming message");
        }
    }
    /**
    ...
    isRealConversation(messages) {
        ...
        const hasMediaWithCaptions = messages.some((msg) => msg.message?.imageMessage?.caption ||
    ...
    async processSyncedContacts(userId, phoneNumber, contacts) {
        try {
            ...
            for (const contact of contacts) {
                ...
                if (!contactNumber || contactNumber === phoneNumber)
                    continue;
                // Use consistent phone number normalization
                const formattedPhone = (0, phoneNumber_1.formatPhoneNumberSafe)(contactNumber);
                if (!formattedPhone) {
                ...
                if (!existingContacts.empty) {
                    ...
                    await existingDoc.ref.update({
                        ...
                        first_name: existingData.first_name === "Unknown" || !existingData.first_name
                            ? firstName || "Unknown"
                            : existingData.first_name,
                        last_name: existingData.last_name === "Unknown" || !existingData.last_name
                            ? lastName || "Unknown"
            ...
            this.logger.info({ userId, phoneNumber, totalSynced: syncedCount }, "Contacts sync completed");
            // Emit event for UI updates
            this.emit("contacts-synced", {
                userId,
                phoneNumber,
            ...
            for (const chat of chats) {
                ...
                if (!existingContacts.empty) {
                    ...
                    const updateData = {
                        last_activity_at: chat.conversationTimestamp
                            ? admin.firestore.Timestamp.fromMillis(chat.conversationTimestamp * 1000)
                            : currentTimestamp,
                        last_modified_at: currentTimestamp,
                        has_had_activity: true,
                        channel: "whatsapp_web",
                    ...
                    if (importListRef) {
                        ...
                        const hasImportList = validLists.some((listRef) => listRef.id === importListRef.id);
                        if (!hasImportList) {
                ...
                if (socket && chat.conversationTimestamp && processedCount < 10) {
                    ...
                    try {
                        ...
                        const historyId = await socket.fetchMessageHistory(50, // Number of messages to fetch
        ...
        try {
            ...
            for (const msg of messages) {
                ...
                if (!contactNumber)
                    continue;
                // Use consistent phone number normalization
                const formattedContactPhone = (0, phoneNumber_1.formatPhoneNumberSafe)(contactNumber);
            ...
            for (const [formattedContactPhone, contactMessages,] of messagesByContact) {
                ...
                const formattedUserPhone = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber) || phoneNumber;
                ...
                if (!existingContacts.empty) {
                    ...
                    const whatsappDisplayName = syncedInfo?.notify ||
                        pushName ||
                        syncedInfo?.verifiedName ||
                        syncedInfo?.name ||
                        null;
                    ...
                    const newContactData = {
                        ...
                        credits_used: 0,
                        last_updated_by: "whatsapp_web_sync",
                        lists: pendingMetadata?.importListRef || importListRef
                ...
                if (lastMessageData && lastMessageRef) {
                    ...
                    if (!lastMessageData.key.fromMe) {
                        lastMessageUpdate.last_incoming_message_at =
                            admin.firestore.Timestamp.fromMillis(lastMessageData.messageTimestamp * 1000);
                    }
    ...
    async reconnect(userId, phoneNumber, attempt = 1) {
        ...
        try {
            ...
            this.logger.error({ userId, phoneNumber, attempt, error }, "Reconnection attempt failed");
    ...
    async createPhoneNumberRecord(userId, phoneNumber, countryCode) {
        try {
            ...
            if (!phoneDoc.exists) {
                ...
                this.logger.info({ userId, phoneNumber, countryCode }, "Created phone number record for WhatsApp Web");
            }
            else {
                ...
                this.logger.info({ userId, phoneNumber }, "Updated existing phone number record");
            }
        }
        catch (error) {
            this.logger.error({ userId, phoneNumber, error }, "Failed to create/update phone number record");
    ...
    hasMemory() {
        ...
        if (process.env.DISABLE_MEMORY_CHECK === "true" ||
