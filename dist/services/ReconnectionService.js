...
class ReconnectionService {
    ...
    async reconnect(userId, phoneNumber, forceNew = false) {
        try {
            ...
            if (success) {
                logger.info({ userId, phoneNumber }, "Successfully reconnected WhatsApp Web session");
                return {
                    success: true,
                    status: "connected",
                ...
                logger.warn({ userId, phoneNumber }, "Failed to reconnect WhatsApp Web session");
                return {
                    success: false,
                    status: "failed",
    ...
    checkRateLimit(userId, phoneNumber) {
        ...
        if (recentAttempts.length >= this.MAX_ATTEMPTS_PER_HOUR) {
            ...
            const retryAfter = Math.ceil((oldestAttempt.timestamp.getTime() +
                this.RATE_LIMIT_WINDOW_MS -
                Date.now()) /
                1000);
