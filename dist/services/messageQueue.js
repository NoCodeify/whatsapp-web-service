...
class MessageQueueService {
  ...
  startProcessing() {
    this.queue.process("send-message", 5, async (job) => {
      ...
      try {
        ...
        // Send message
        const messageKey = await this.connectionPool.sendMessage(
          userId,
          phoneNumber,
          toNumber,
          content,
  ...
  calculatePriority(message) {
    ...
    if (message.metadata?.priority === "high") return 10;
    if (message.metadata?.priority === "low") return 1;
    // Campaign messages get medium-high priority
    if (message.metadata?.campaignId) return 7;
    // Template messages get medium priority
    if (message.metadata?.templateId) return 5;
    // Default priority
    return 3;
  }
  /**
  ...
  async storeJobReference(jobId, message) {
    try {
      const jobRef = this.firestore.collection("message_queue_jobs").doc(jobId);
      await jobRef.set({
        jobId,
        userId: message.userId,
  ...
  async notifyFailure(job) {
    try {
      ...
      this.logger.error(
        { error, jobId: job.id },
        "Failed to notify about failure",
      );
    }
  }
  /**
