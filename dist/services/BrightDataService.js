...
class BrightDataService {
    ...
    async syncStaticIPs() {
        try {
            ...
            this.logger.warn({ error: error.message }, "Failed to sync from API, using session-based approach");
    ...
    async testConnection(phoneNumber) {
        try {
            ...
            const response = await axios_1.default.get("https://geo.brdtest.com/welcome.txt", {
                proxy: {
            ...
            this.logger.error({ error: error.message }, "ISP proxy connection test failed");
            return false;
        }
    }
    /**
