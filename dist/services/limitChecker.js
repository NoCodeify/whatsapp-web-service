...
const logger = (0, pino_1.default)({ name: "LimitChecker" });
...
class LimitChecker {
     ...
     */
    async checkIfNewContact(userId, recipientNumber) {
        try {
     ...
     */
    resetDailyCountersIfNeeded(usage) {
     ...
     */
    async checkLimits(userId, phoneNumber, recipientNumber) {
        try {
            // Get user settings
            ...
            const settings = userData?.whatsapp_web_settings || {
            ...
            let usage = phoneData?.whatsapp_web_usage || {
            ...
            if (isNewContact) {
                ...
                logger.info({ userId, phoneNumber, delayMs, isNewContact }, `Applying delay of ${Math.round(delayMs / 1000)}s for new contact`);
            }
...
//# sourceMappingURL=limitChecker.js.map