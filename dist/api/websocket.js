...
function createWebSocketHandlers(io, connectionPool, _sessionManager) {
    ...
    io.use((socket, next) => {
        ...
        if (phoneNumber) {
            const formatted = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber);
            if (formatted) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("subscribe:connection", (data) => {
            ...
            const phone = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber);
            if (!phone) {
        ...
        socket.on("messages:read", async (data) => {
            ...
            if (!phone || !messageIds || !chatJid || !userId)
                return;
            const connection = connectionPool.getConnection(userId, phone);
            if (connection && connection.state.connection === "open") {
    ...
    connectionPool.on("qr-generated", (data) => {
    ...
    connectionPool.on("connection-update", (data) => {
    ...
    connectionPool.on("message-received", (data) => {
    ...
    connectionPool.on("message-status", (data) => {
    ...
    connectionPool.on("typing-indicator", (data) => {
    ...
    connectionPool.on("presence-update", (data) => {
    ...
    // History sync events
    connectionPool.on("history-synced", (data) => {
    ...
    connectionPool.on("contacts-synced", (data) => {
    ...
    connectionPool.on("messages-synced", (data) => {
        const { userId, phoneNumber, count } = data;
        // Emit progress update
    ...
    connectionPool.on("message-stored", (data) => {
    ...
    connectionPool.on("sync:started", (data) => {
    ...
    connectionPool.on("sync:progress", (data) => {
