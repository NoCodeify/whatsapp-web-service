...
const limitChecker_1 = require("../services/limitChecker");
const pino_1 = __importDefault(require("pino"));
const phoneNumber_1 = require("../utils/phoneNumber");
const logger = (0, pino_1.default)({ name: "API" });
const limitChecker = new limitChecker_1.LimitChecker();
...
function createApiRoutes(connectionPool, sessionManager, proxyManager, connectionStateManager, reconnectionService) {
    ...
    router.post("/sessions/initialize", async (req, res) => {
        try {
            ...
            const formattedPhone = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber, countryCode);
            if (!formattedPhone) {
     ...
     */
    router.get("/sessions/:userId/qr", async (req, res) => {
        try {
            ...
            if (format === "image") {
                // Return as PNG image
                const qrImage = await qrcode_1.default.toBuffer(connection.qrCode, {
                    type: "png",
                    width: 512,
                    margin: 2,
                });
                ...
                // Return as SVG
                const qrSvg = await qrcode_1.default.toString(connection.qrCode, {
                    type: "svg",
                    width: 512,
                });
                ...
                const qrDataUrl = await qrcode_1.default.toDataURL(connection.qrCode, {
                    width: 512,
                    margin: 2,
                });
     ...
     */
    router.get("/sessions/:userId/status", async (req, res) => {
        try {
            const { userId } = req.params;
            const phoneNumber = req.query.phoneNumber;
            if (!phoneNumber) {
    ...
    if (reconnectionService) {
         ...
         */
        router.get("/sessions/can-reconnect", async (req, res) => {
            try {
                const phoneNumber = req.query.phoneNumber;
                const userId = req.user.userId;
                if (!phoneNumber) {
         ...
         */
        router.get("/sessions/reconnect-status", async (req, res) => {
            try {
                const phoneNumber = req.query.phoneNumber;
                const userId = req.user.userId;
                if (!phoneNumber) {
         ...
         */
        router.get("/sessions/reconnection-stats", async (_req, res) => {
            try {
     ...
     */
    router.post("/messages/check-limits", async (req, res) => {
        try {
    ...
    router.post("/messages/send", async (req, res) => {
        ...
        const correlationId = req.correlationId || `msg_${Date.now()}`;
        try {
            ...
            const formattedPhone = (0, phoneNumber_1.formatPhoneNumberSafe)(phoneNumber);
            const formattedToNumber = (0, phoneNumber_1.formatPhoneNumberSafe)(toNumber);
            ...
            logger.info({
                ...
                sendDuration,
                totalDuration,
                creditsUsed: limitCheck.usage?.messagesUsedToday ||
    ...
    router.post("/messages/read", async (req, res) => {
        try {
            ...
            // Mark as read
            await connection.socket.readMessages(messageIds.map((id) => ({
                remoteJid: chatJid,
                id,
     ...
     */
    router.get("/proxy/locations", async (req, res) => {
        try {
     ...
     */
    router.post("/connections/recover", async (req, res) => {
        try {
